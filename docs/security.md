...
- **마스터 키 관리** (X1 결정): 앱 마스터 키는 환경 변수 `APP_SECRET_KEY`를 **우선** 사용하고, 미설정 시 최초 기동에서 권한 600의 `~/.selectai/secret.key`를 **자동 생성**하되 콘솔에 경고를 출력한다. 암호화 데이터(`connections.json`)와 키 파일은 **별도 파일**로 분리하며, 키 없이 평문으로 저장하는 폴백은 **금지**한다 (자동 생성은 허용하되 평문 폴백은 불허 — 데모 도구의 설치 간편성과 최소 보안선을 동시에 충족). 회전 정책은 P2 검토. "키-데이터 분리, 평문 폴백 금지" 두 원칙은 불변 조건이다.
...
- **읽기 전용 게이트(필수)**: runsql/챗봇 턴이 반환·실행하는 SQL에 대해 백엔드가 SELECT 문 여부를 검사한다. SELECT 외 구문(INSERT/UPDATE/DELETE/DROP/ALTER/TRUNCATE 등)이 감지되면 자동 실행을 차단하고 경고와 함께 수동 확인을 요구한다. (admin 접속 확정 사항에 대한 보상 통제 — DB 계정 권한 축소가 불가능하므로 앱 계층에서 차단)
...
- 자연어 프롬프트로 "drop table을 만들어 달라"는 식의 유도(직접 또는 COMMENT 텍스트를 통한 간접 주입)가 가능하다. §3.3의 읽기 전용 게이트가 최종 방어선이며, COMMENT 입력 폼에는 "코멘트는 LLM 프롬프트에 포함된다"는 안내를 표시한다 (comments=true 시 증강 프롬프트에 포함됨 — 레퍼런스 §7).
