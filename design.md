...
6. **대상 테이블은 사용자가 선택(원칙)**: 프로파일의 `object_list`(NL2SQL 대상 객체)는 **인증한 커넥션 사용자가 DB에서 볼 수 있는 테이블 중 사용자가 직접 고른 것**으로 구성한다(PG-03a 스키마/테이블 브라우저). 앱이 제공하는 SH·무비 모호 스키마는 데모 편의용 프리셋일 뿐이며, 사용자는 자신이 접속한 스키마의 임의 테이블을 대상으로 선택해 프로파일을 만들 수 있다 (X3 결정).
