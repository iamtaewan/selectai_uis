...
const outputDir = path.resolve(root, "design-output", "png");
fs.mkdirSync(outputDir, { recursive: true });

const pages = [
  ["PG-00", "PG-00-onboarding.png", "온보딩"],
  ["PG-01", "PG-01-connections.png", "커넥션"],
  ["PG-02", "PG-02-permissions.png", "권한 점검"],
  ["PG-03", "PG-03-profiles.png", "프로파일 목록"],
  ["PG-03a", "PG-03a-profile-editor.png", "프로파일 생성"],
  ["PG-03b", "PG-03b-profile-detail.png", "프로파일 상세"],
  ["PG-04", "PG-04-playground.png", "플레이그라운드"],
  ["PG-05", "PG-05-chat.png", "챗봇"],
  ["PG-05a", "PG-05a-chat-history.png", "대화 이력"],
  ["PG-06", "PG-06-enrichment.png", "증강 비교"],
  ["PG-07", "PG-07-dashboard.png", "대시보드"],
  ["PG-08", "PG-08-settings.png", "설정"],
];
...
async function captureDesktop(browser) {
  ...
  for (const [id, file] of pages) {
    const url = `${base}?page=${encodeURIComponent(id)}`;
...
async function mobileCheck(browser) {
  ...
  const results = [];
  for (const id of ids) {
    await page.goto(`${base}?page=${encodeURIComponent(id)}`, { waitUntil: "networkidle" });
...
async function makeContactSheet() {
  ...
  await sharp({
    ...
    .png()
    .toFile(path.join(outputDir, "INDEX-contact-sheet.png"));
  console.log("captured INDEX-contact-sheet.png");
}
...
(async () => {
  ...
  try {
    ...
    const failed = mobile.filter((r) => !r.ok);
